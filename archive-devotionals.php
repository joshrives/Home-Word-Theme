...
<?php
	...
	if($_SESSION['cat'] === 'families') :
		...
		if ( $loop->have_posts() ) : ?>
				...
				<h4 class="archive-title">Devotionals for Families</h4>
				<div class="group">
...
<?php
	elseif($_SESSION['cat'] === 'church') :
		...
		if ( $loop->have_posts() ) : ?>
				...
				<h4 class="archive-title">Devotionals for Church Leaders</h4>
				<div class="group">
				<?php while ( $loop->have_posts() ) : $loop->the_post();?>
					<div class="entry-excerpt">
						<?php get_template_part( 'content', 'archive' ); ?>
...
<?php else :
		$args = array( 'post_type' => 'devotionals', 'posts_per_page' => 4);
		$loop = new WP_Query( $args );
