...
?>
<div class="your-order group" id="order_review">
	...
	<ul class="order-list">
				...
				if (sizeof($woocommerce->cart->get_cart())>0) :
					foreach ($woocommerce->cart->get_cart() as $cart_item_key => $values) :
						...
						if ($_product->exists() && $values['quantity']>0) :
							echo '
								<li class="group ' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
									...
									'</div>
									<div class="order-amount">' . apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $cart_item_key ) . '</div>
								</li>';
						endif;
					endforeach;
				endif;
			...
			<li class="group divider">
				...
				<div class="order-amount"><?php echo $woocommerce->cart->get_cart_subtotal(); ?></div>
			</li>

			<?php if ( $woocommerce->cart->get_discounts_before_tax() ) : ?>

			<li class="group">
				<div class="order-label"><?php _e( 'Cart Discount', 'woocommerce' ); ?></div>
				<div class="order-amount">-<?php echo $woocommerce->cart->get_discounts_before_tax(); ?></div>
			</li>
			...
			<?php if ( $woocommerce->cart->needs_shipping() && $woocommerce->cart->show_shipping() ) : ?>
				...
				<li class="group">
					<div class="order-label"><?php _e( 'Shipping', 'woocommerce' ); ?></div>
					<div class="order-amount"><?php woocommerce_get_template( 'cart/shipping-methods.php', array( 'available_methods' => $available_methods ) ); ?></div>
				</tli>
			...
			<?php foreach ( $woocommerce->cart->get_fees() as $fee ) : ?>
				...
				<li class="group fee-<?php echo $fee->id ?>">
					<div class="order-label"><?php echo $fee->name ?></div>
					<div class="order-amount"><?php
				...
				if ( $woocommerce->cart->tax_display_cart == 'excl' ) {
					foreach ( $woocommerce->cart->get_tax_totals() as $code => $tax ) {
						echo '<li class="group tax-rate-' . $code . '">
							<div class="order-label">' . $tax->label . '</div>
							<div class="order-amount">' . $tax->formatted_amount . '</div>
						</li>';
					}
				}
			?>
			...
			<li class="group">
				<div class="order-label"><?php _e( 'Order Discount', 'woocommerce' ); ?></div>
				<div class="order-amount">-<?php echo $woocommerce->cart->get_discounts_after_tax(); ?></div>
			</li>
